    ...
    <div className="modal-layer guide-layer" role="dialog" aria-modal="true" aria-label={`${guide.name}动作教学`}>
      <div className="guide-shell">
        ...
        <main className="guide-content">
          ...
          <section className="guide-section"><span className="section-kicker">器械与起始位置</span><h3>先把器械调对</h3><ul className="check-list">{guide.equipmentSettings.map((item) => <li key={item}><Check />{item}</li>)}</ul><ol className="number-list">{guide.setup.map((item) => <li key={item}>{item}</li>)}</ol></section>
          <section className="guide-section"><span className="section-kicker">动作执行</span><h3>从设置到完成</h3><ol className="number-list">{guide.steps.map((item) => <li key={item}>{item}</li>)}</ol><div className="cue-grid"><div><span>呼吸</span><p>{guide.breathing}</p></div><div><span>节奏</span><p>{guide.tempo}</p></div><div><span>活动范围</span><p>{guide.range}</p></div></div><div className="cue-strip">{guide.cues.map((cue) => <span key={cue}>{cue}</span>)}</div></section>

          <section className="guide-section"><span className="section-kicker">常见错误</span><h3>两种最常见错误</h3><div className="mistake-grid">{guide.mistakes.slice(0, 2).map((item, index) => <article key={item.title} className="mistake-card"><ExerciseVisual guide={guide} mistakeIndex={index} phaseIndex={1} /><h4>{item.title}</h4><p><strong>修正：</strong>{item.correction}</p></article>)}</div></section>

          <section className="guide-section"><span className="section-kicker">身体反馈</span><h3>知道什么时候继续，什么时候停</h3><div className="feel-card good"><Check size={20} /><div><strong>应该感受到</strong><p>{guide.intendedFeel}</p></div></div><div className="feel-card danger"><Warning size={20} /><div><strong>立即停止</strong><p>{guide.stopSignals.join("；")}</p></div></div></section>

          <section className="guide-section"><span className="section-kicker">替代与安全</span><h3>器械被占也能继续</h3><div className="alternatives"><div><span>降阶</span>{guide.regressions.map((item) => <b key={item}>{item}</b>)}</div><div><span>替代</span>{guide.alternatives.map((item) => <b key={item}>{item}</b>)}</div></div><div className="safety-note"><Warning /><p>{guide.safety}</p></div></section>
